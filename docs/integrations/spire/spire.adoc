...
Create namespace, note this time we are explicitly
...
EOF
----
Patch the default ClusterSPIFFEID `zero-trust-workload-identity-manager-spire-default`
[source,bash]
----
# patch the ClusterSPIFFEID here disable for test-1 and for test-2
----
Create ClusterSPIFFEID for service in test-1 and test-2 curl service in
[source,bash]
----
cat <<EOF | kubectl apply -f -
...
kubectl exec $CURL_POD -n test-2 \
  -it -- /tmp/spire-agent api fetch x509 -socketPath /tmp/spiffe-socket/socket  -write /tmp
# making direct request to the service withing the mesh from outside mesh service
kubectl exec $CURL_POD -n test-2 \
  -it -- curl -s -o /dev/null -w "%{http_code}" --insecure --key /tmp/svid.0.key --cert /tmp/svid.0.pem  https://httpbin.test-1.svc
----
A note about `--insecure` flag.
The reason for making an insecure request, is because `curl`
does not natively support SPIFFE protocol.
Which means, it's not capable to validate httpbin x509 SVID.
For production use cases, you either should implement
https://github.com/spiffe/go-spiffe[Spiffe SKD]
or use some third parity solutions who implement SPIFFE protocol
like https://github.com/ghostunnel/ghostunnel[Ghostunnel].
More info https://spiffe.io/docs/latest/deploying/svids/[here]
Another option to overcome the compatibility problem could be
DNS SANs into SVID. This can be accomplished by modifying
Spire Agent configuration and enable this feature.
